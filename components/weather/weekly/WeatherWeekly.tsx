...
import { adaptTomorrow } from "@/lib/weather";
import { AdaptForecast, AdaptTomorrow } from "@/types/forecast.types";
...
export function WeatherWeekly({ weekly }: WeatherWeeklyProps) {
    const sevenday = (
        <div className="flex flex-col pb-2">
            <Accordion defaultValue={["Today"]} multiple className="gap-4">
                {weekly.map((element) => {
                    const {
                        dt,
                        sunrise,
