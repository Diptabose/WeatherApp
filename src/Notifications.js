...
function getPublicKeyAndReturnURL(){
  const publicKey = 'BEyjBD8bR-ZsO9F0vFyoBKfgKNg1SdX8fr_rfVhmhwXZYzc-B9KJepeyICjLdPkk9EUCRBLvHawNve3OZ1MWFhw';
...
export async function unSubscribeNotifications(){
 ...
 let endpoint=null;
  try{
    const applicationServerKey = await getPublicKeyAndReturnURL();
  ...
  try{
  const resp=await fetch('https://weatherapp-api1251.herokuapp.com/send/deleteendpoint',{
    method:'POST',
...
//Function to send notifications... generate endpoint and takes loaction as parameter to send a stringified json object to the server and register the push endpoint in the database.
  ...
  try{
     if(window.localStorage.getItem('notif')==='denied'){
      ...
      body = {location:userLocation,endpoint:endpoint}
       const resp= await fetch('https://weatherapp-api1251.herokuapp.com/send/endpoint',
       {
         method:'POST',
         headers: {'Content-Type':'application/json'},
